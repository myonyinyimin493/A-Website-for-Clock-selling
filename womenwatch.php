...
function womenshow() {
    ...
    $showwomen = women($start);
    $heros = array();
    foreach ($showwomen as $row) {
    ...
    <div class="row">
        ...
        foreach ($heros as $hero) {
            ...
            echo '<form method="POST" action="women.php?action=add&id=' . $hero['id'] . '">';
    ...
    if ($numberOfPages > 1) {
        ...
        for ($i = 0; $i < $numberOfPages; $i++) {
            echo "<li class=page-item ><a class=page-link href='women.php?start=$start' >$pageNumber</a></li>";
            $start+=$limit;
            $pageNumber++;
        }
        echo "</ul>";
